 ...
 * 同期取れていない、バグあり
 */
public class UpDownSort8<E extends Comparable> {
	private E[] array;
	...
	private void init(E[] array) {
		...
		//long stop = System.currentTimeMillis();
		//System.out.println("initTIME : "+(stop - start));
...
//						merge(left,mid,right);
						...
						if(isSorted[left - 1].get() != isSorted[left].get()){  //左のソートが終わっていれば
							isSorted[left].set(!isSorted[left].get());
							//ソート範囲を拡大しmergeする
							mid = left - 1;
							left = posPointer[mid];
						...
						if(isSorted[right].get() != isSorted[right + 1].get()){	//右のソートが終わっていれば
							//ソート範囲を拡大しmergeする
							isSorted[right].set(!isSorted[right].get()); //同期のため
							mid = right;
