 ...
 */
public class UpDownSort10_2<E extends Comparable> {
	//private E[] array;
	//private E[] buff;
		...
		private void expanseMerge() {
			do{
				...
				if(right == arrayLength - 1){
					...
					if(checkSideIsSorted(left,right,false)){  //左のソートが終わっていれば
						mid = left - 1;
						left = posPointer[mid];
					...
					if(checkSideIsSorted(left,right,true)){	//右のソートが終わっていれば
						mid = right;
					...
					if(checkSideIsSorted(left,right,false)){  //左のソートが終わっていれば
						...
						left = posPointer[mid];
						merge(array,buff,left,mid,right);
					}else if(checkSideIsSorted(left,right,true)){	//右のソートが終わっていれば
						mid = right;
						right = posPointer[mid+1];
						merge(array,buff,left,mid,right);
	...
	private synchronized boolean checkSideIsSorted(int left,int right,boolean dir){
