 ...
 */
public class UpDownSort10<E extends Comparable> {
	//private E[] array;
	//private E[] buff;
	...
	public long sort(E[] array){
		...
		if(MyArrayUtil.checker(array)){
			System.out.println("OOOKKKKKK");
		}else{
			...
			do{
				...
				if(right == arrayLength - 1){
					...
					if(checkLeftSideIsSorted(left,right)){  //左のソートが終わっていれば
						...
						//System.out.println(Thread.currentThread().getName()+ "RIGHT  left: "+left+"  right : "+right);
					}else{
						//System.out.println(Thread.currentThread().getName()+"left "+left+"  right : "+right+" : END1");
						//printBoolean();
						return;//終了
					}
					...
					if(checkRightSideIsSorted(right,left)){	//右のソートが終わっていれば
						...
						merge(array,buff,left,mid,right);
						//System.out.println(Thread.currentThread().getName()+ "LEFT   left: "+left+"  right : "+right);
					}else{ //右のソートが終わっていなければ終了
						//System.out.println(Thread.currentThread().getName()+" left "+left+"  right : "+right+" : END2");
						//printBoolean();
						return;//終了
					}
					...
					if(checkLeftSideIsSorted(left,right)){  //左のソートが終わっていれば
						...
						left = posPointer[mid];
						merge(array,buff,left,mid,right);
						//System.out.println(Thread.currentThread().getName()+ "LEFT MD  left: "+left+"  right : "+right);
						...
						//System.out.println(Thread.currentThread().getName()+ "RIGHT MD  left: "+left+"  right : "+right);
					}else{ //どちらのソートも終わっていなければ

						//System.out.println(Thread.currentThread().getName()+"left"+left+"  right : "+right+" : END3");
						//printBoolean();
						return;
					}
				}
				...
				if(left == 0 && right == arrayLength - 1){	//最後のソートであったら終了
					//System.out.println(Thread.currentThread().getName()+"left"+left+"  right : "+right+" : END4444444444");
					//executor.shutdownNow();
					//MyArrayUtil.print2(array);
					//updateAll(left,right);	//isSorted,posPointer,numの更新
				...
				//endMergeIsSorted(left,right);	//IsSortedの更新
				//updateAll(left,right);	//isSorted,posPointer,numの更新
			}while(true);
		}
