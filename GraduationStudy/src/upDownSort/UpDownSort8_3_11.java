 ...
 */
public class UpDownSort8_3_11<E extends Comparable> {
	//private E[] array;
	//private E[] buff;
...
//						k+=i - lastPos;
					...
					if(add >= 4){
						i = binarySearch2(array,endPos,lastPos,array[j],lastPos);
		...
		public int binarySearch2(E[] array,int left,int right,E key,int end){
			...
			if(array[mid].compareTo(key) < 0)
				return binarySearch2(array,left,mid-1,key,end);
			else if(array[mid].compareTo(key) == 0)
				return binarySearch2(array,left+1,right,key,end);
			else
			    return binarySearch2(array,mid+1,right,key,end);
