 ...
 */
public class UpDownSort8_3_3<E extends Comparable> {
	//private E[] array;
	private E[] buff;
	...
	public void createTasks(E[] array){
			...
			if(array[right].compareTo(array[right+1]) <= 0){	//昇順なら
				...
				while(array[right].compareTo(array[right+1]) <= 0){  //どこまで昇順か調べる
					if(right+1 >= arrayLength-1){	//最後まで昇順であれば
						if(unitCount == 0){
							settingTask(array, left,right+1,REST); //仕事をセットする
							return;
						}else{
							right++;	//次に進める
							break;
				...
				while(array[right].compareTo(array[right+1]) > 0){//どこまで降順か調べる
					if(right+1 >= arrayLength-1){	//最後まで降順であれば
						if(unitCount == 0){		//二つのセットでなければ
							settingTask(array,left,right+1,ALLREVERSE);  //仕事をセットする
							return;
						}else{	//二つのセットであれば
							right++;	//次に進める
							break;
			...
			if(unitCount == 0){  //一つ目の昇順or降順を調べたとき
				...
				if(right+1 == arrayLength - 1){	//次の一つの要素が配列の終わりなら
					if(pattern == 1) //patternで判定する
						...
						pattern = DOWNUP;
					settingTask(array,left,mid,right+1,pattern);  //MergeSortWorkerに仕事を投げる
					return;
				}
			}else{	//二つのセットができたとき
				settingTask(array, left,mid,right,pattern);  //MergeSortWorkerに仕事を投げる
				if(right+1 == arrayLength-1){  //次の一つの要素が配列の終わりなら
					settingTask(array, right+1,right+1,REST);  //MergeSortWorkerに仕事を投げる
					return;
				}
				pattern = 1; //初期化
				unitCount = 0;	//初期化
	...
	//二つセットで仕事がある場合
	private void settingTask(E[] array, int left,int mid,int right,int pattern){
	...
	private void settingTask(E[] array, int pos1,int pos2,int pattern){
	...
	class MergeSortWorker implements Runnable{
		...
		public MergeSortWorker(E[] array,E[] buff,int left,int mid,int right,int num,int pattern){
			this.num = num;
