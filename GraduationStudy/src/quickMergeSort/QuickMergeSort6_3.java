...
public class QuickMergeSort6_3<E extends Comparable> {
	...
	public void merge(E[] array, E[] buff, int left,int mid,int right){
		...
		while(i <= mid && j <= right){
			...
			if(array[i].compareTo(array[j]) < 0){
				...
				System.arraycopy(array,lastPos,buff,k,i - lastPos+1);
				k+=i - lastPos+1;

				i++;

				lastPos = i;
				while(array[i].compareTo(array[j]) < 0 && i <=mid){
					buff[k++] = array[i++];
				}
...
//				k+=i - lastPos;
	...
	private synchronized void bprint(E[] array, int left, int right){
		System.out.print("before  ");System.out.println("left : "+ left+"  right :"+right);
		MyArrayUtil.print(array);
	}
	private synchronized void print(E[] array, int left, int right){
